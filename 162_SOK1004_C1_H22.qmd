...
ryddig_df <- as_tibble(df$dataset)

ryddig_df <- rename(ryddig_df, var = ContentsCode, verdi = value, tid = Tid)
  
ryddig_df$tid <- as.numeric(ryddig_df$tid)
...
ryddig_df %>%
  ...
  ggplot +
  geom_line(var = "BNP", (aes(x=tid, y=verdi, color=var))) +
  geom_line(var = "MEMOBNP", (aes(x=tid, y=verdi, color=var)))

```

*\[Besvar oppgave 1b her\]*

Vi kan på grafen at den reelle BNP utviklingen (Den blå grafen, MEMOBNP) har hatt en mye mer avtagende økning. Sammenlignet med den nominelle utviklingen (den røde grafen, BNP). Vi kan derfor mye lettere se hvordan inflasjon har påvirket den stadige økningen i Norges bruttonasjonalprodukt.
...
Ved bruk av kommandoen `pivot_wide()` gjorde vi data mer oversiktlig. Kommandoen ordnet BNP og MEMOBNP som variabler isteden for verdier. Isteden for at hvert år skal ha to observasjoner spredt på to rader, kan vi samle begge obersvasjonene i samme rad. Slik som vi gjorde med `df_wide`..
...
BNP_delta = ryddig_df_wide$BNP - ryddig_df_wide$LBNP

MEMOBNP_delta = ryddig_df_wide$MEMOBNP - ryddig_df_wide$LMEMOBNP

ryddig_df_wide <- ryddig_df_wide %>%
  mutate(DBNP=100*(BNP_delta/ryddig_df_wide$LBNP)) %>%
  mutate(DMEMOBNP=100*(MEMOBNP_delta/ryddig_df_wide$LMEMOBNP))
```
...
ryddig_df_wide %>%
  filter(tid %in% (1971:2021)) %>%
  ggplot +
  geom_point(aes(x=tid, y=DBNP, color = "Reel BNP")) +
  geom_point(aes(x=tid, y=DMEMOBNP, color = "Nominell BNP")) +
  geom_smooth(aes(x=tid, y=DBNP, color = "Reel BNP")) +
  geom_smooth(aes(x=tid, y=DMEMOBNP, color = "Nominell BNP")) +
  ggtitle("Prosentvis vekst i nominelt og reelt BNP") +
  xlab("Tid") +
  ylab("Prosentendring")
